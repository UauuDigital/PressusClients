...
function pdfHTML({ form, quote, venue, dateStr, coupleStr, refNum, today, t, lang }) {
  ...
  const linesRows = [
    ...
    ...quote.extrasLines.map(e => `<tr><td class="td-l">${getExtraLabel(e.id, lang) || e.label}${e.isMandatory ? ` <span style="font-size:10px;color:#999;font-style:normal">${t.pdfMandatory}</span>` : ''}${e.priceDetail ? `<div class="td-sub">${e.priceDetail}</div>` : ''}</td><td class="td-r">${eur(e.computedPrice)}</td></tr>`),
  ].join('');
...
body{font-family:'Cormorant Garamond',Georgia,serif;font-weight:300;font-size:15px;line-height:1.35;color:#1c1c1a;background:#fff;-webkit-font-smoothing:antialiased}
.wrap{max-width:620px;margin:0 auto;padding:28px 34px 30px;min-height:100%;display:flex;flex-direction:column}
...
.divider{height:1px;background:#eaeced;margin:16px 0}
.meta-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px 28px;margin-bottom:18px}
.meta-label{font-family:system-ui,sans-serif;font-size:9px;letter-spacing:0.16em;text-transform:uppercase;color:#7e8c8d;margin-bottom:3px}
.meta-val{font-size:15px;font-weight:400}
.meta-val-i{font-size:15px;font-weight:300;font-style:italic}
.tbl-label{font-family:system-ui,sans-serif;font-size:9px;letter-spacing:0.16em;text-transform:uppercase;color:#7e8c8d;margin-bottom:6px}
table{width:100%;border-collapse:collapse}
th{font-family:system-ui,sans-serif;font-size:9px;letter-spacing:0.13em;text-transform:uppercase;color:#7e8c8d;font-weight:400;padding-bottom:7px;border-bottom:1px solid #1c1c1a;text-align:left}
th:last-child{text-align:right}
.td-l{padding:7px 0;border-bottom:1px solid #eaeced;font-size:13px;font-weight:300;color:#515856;vertical-align:top}
.td-sub{font-family:system-ui,sans-serif;font-size:9px;color:#7e8c8d;margin-top:2px;letter-spacing:0.04em}
.td-r{padding:7px 0;border-bottom:1px solid #eaeced;text-align:right;font-size:13px;font-weight:400;white-space:nowrap;vertical-align:top}
...
.total-td{padding:10px 0;border-top:2px solid #1c1c1a}
.t-label{font-family:system-ui,sans-serif;font-size:9px;letter-spacing:0.18em;text-transform:uppercase;color:#7e8c8d;margin-bottom:3px}
.t-amount{font-size:25px;font-weight:300}
.t-pp{font-size:11px;font-style:italic;color:#7e8c8d;margin-top:2px}
.notes{margin-top:12px;padding:10px 12px;background:#f5f2ed;border-radius:4px;font-size:11px;font-style:italic;color:#515856;line-height:1.35}
.push-bottom{flex:1 1 auto}
.footer{margin-top:14px;padding-top:10px;border-top:1px solid #eaeced;display:flex;justify-content:space-between;align-items:flex-end;gap:16px}
...
.footer-r{font-family:system-ui,sans-serif;font-size:9px;color:#7e8c8d;text-align:right;line-height:1.4}
@page{size:A4;margin:0}
@media print{
  html,body{width:210mm;height:297mm;overflow:hidden}
  body{-webkit-print-color-adjust:exact;print-color-adjust:exact}
  .wrap{width:210mm;max-width:none;min-height:297mm;transform:scale(.92);transform-origin:top left}
}
</style></head><body>
<div class="wrap">
  <div class="pdf-top">
